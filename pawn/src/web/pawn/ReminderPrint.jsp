...
<html:html>
	<head>
		...
		<script type="text/javascript">								
			...
			var url = 'reminderPrintService.do';
			...
			function print(){	
				var str = '';
				var reminderDate = $('endDate').value;
				alert(reminderDate);
				for(var i=0;i<grid.getRowCount();i++) { 
		        ...
		        if(str == '')
					...
					var data = url + '?dispatch=print&reminders=' + str.substring(3) + "&reminderDate=" + reminderDate;
					theHeight=500;
					theWidth=800;
					...
					window.open(data,"PrintPreview",features);
					clearAll();
				}
			...
			function getReminderParaSerchData(){
					...
					mySearchRequest.callback = function(responseText, responseStatus, responseXML) {
						if (responseStatus==200) {						
				    	    ...
				    	    alert(responseStatus + ' -- 1Error Processing Request'); 
					    }
					}
					mySearchRequest.update(data,'POST');
			}
	...
	<body>
		...
		<!-- Create record -->
		<logic:equal name="reminderprint" property="action" value="create">
			<html:form action="reminderPrintService.do">
							...
							<table cellpadding="0" cellspacing="0">
								<tr>
									<td style="width: 140px" align="right">
										<bean:message bundle="lable" key="screen.reminder.date"/>&nbsp;
									</td>
									<td>
										<html:text property="endDate" size="15" maxlength="12" styleId="endDate" 
													onkeypress="DateFormat(this,event);" 
													onfocus="clearDivision('divEndDate')" 
													onkeyup="DateFormat(this,event);" />
										<img src="images/none.gif" width="5px">
										<input type="button" value="..." onclick="return showCalendar('endDate');" />
