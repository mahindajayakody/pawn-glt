		...
		<script type="text/javascript">
			...
			function createEditBrowser(myColumns,gridName,dataFormat){
			    ...
			    obj.onCellValueChanged = function(value, col, row) {
			    	if(col==0){
				    	if(!validateReleaseItems(value,col, row)){
				    		//return "error";
				    		obj.setCellValue(false,0,row);
				    	}
				    }
				...
				if(value==true){
					...
					for(i=0;i<gridTicket.getRowCount();i++){
						if(gridTicket.getCellValue(0,i)==true){
							payToAmt = payToAmt + unformatNumber(gridTicket.getCellValue(3,i))*1/gridTicket.getCellValue(4,i)*gridTicket.getCellValue(5,i);
						}
					}
					var receiptAmt  = unformatNumber($('totalreceipts').value)*1;
					var interestAmt = $('interestAmt').value*1;

					if((interestAmt + payToAmt ) > receiptAmt ){
						alert('Not enough receipt amount.');
					...
					if (responseStatus==200) {
						...
						for(i=0;i<dueFromList.length;i++){
							...
							if(dueFromList[i][4]=='INT')
								$('interestAmt').value = unformatNumber(dueFromList[i][1])*1;
						}
			...
			function confirms(){
				...
				data = "dispatch=renewTicket&ticketId=" + $('ticketId').value + "&ticketNumber=" + $('ticketNumber').value;
				var mySearchRequest = new ajaxObject(url);
				mySearchRequest.callback = function(responseText, responseStatus, responseXML) {
					if (responseStatus==200) {
						var message =  eval('(' + responseText + ')');
						...
						<td>
							<table class="InputTable">
								...
								<tr>
									...
									<td>
										<input type="text" size="71" maxlength="100" id="clientAddress" readonly="readonly" class="READONLYINPUT" />
									</td>
								</tr>
								<tr height="2px"></tr>
							</table>
									...
									<td colspan="4" align="center">
										<script>
											var myColumns = ["<bean:message bundle="lable" key="screen.select"/>",
															 ...
															 "<bean:message bundle="lable" key="screen.noofreleseitem"/>"];
