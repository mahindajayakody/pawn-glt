...
import com.dass.pawning.domain.CancelDueReceipt;
...
public class ReceiptDAOImpl extends TransactionDAOSupport implements ReceiptDAO {
	...
	public String createReceipt(UserConfig userConfig,Receipt receipt){
		...
		List<DueType> dueTypeList = getSession().createCriteria(DueType.class)
		...
		String serialNumber = decimalFormat.format(getSerialValue(userConfig, "RCP" , ticket.getProductId()));
		...
		for(DueType dueType:dueTypeList){
			...
			if(receiptAmt > 0 && dueMap.containsKey(dueType.getDueTypeId())){
				...
				if(dueType.getDueType().equals("INT") && dueFrom.getBalanceAmount()>receiptAmt)
	...
	public ReceiptDTO getReceiptTicketData(UserConfig userConfig,int ticketId ){
		...
		List<DueFrom> dueList = getSession().createCriteria(DueFrom.class)
