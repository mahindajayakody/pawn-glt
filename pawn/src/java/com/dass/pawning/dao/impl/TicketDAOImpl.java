...
import com.dass.pawning.domain.DueTo;
...
//		List<ParameterValue> paraMinIntList = paraMinIntCriteria.list();
...
//		}
		...
		if(branch.getFundAvailable()<ticket.getPawnAdvance()){
			throw new CommonDataAccessException("errors.noenoughfunds");
		}
		//double minInterest = Integer.parseInt(paraMinIntList.get(0).getParaValue());
		...
		for(InterestSlab slab:slabList){
			dueFormInt += ticket.getPawnAdvance() * slab.getRate() / 1200 ;
...
//		//this should change
		...
		ticket.setPawnAdvance(ticket.getPawnAdvance());
		...
		if (maxAdvance < ticket.getPawnAdvance()){
			...
			expCal.setTime(ticket.getTicketDate());
			expCal.add(Calendar.MONTH, ticket.getPeriod());
			ticket.setTicketStatusId(TicketStatusEnum.ACTIVE.getCode());
			ticket.setTicketExpiryDate(expCal.getTime());
			ticket.setApprovedBy(userConfig.getUserId());
			branch.setFundAvailable(branch.getFundAvailable()-ticket.getPawnAdvance());
...
//		dueToCriteria.add(Restrictions.eq("companyId",userConfig.getCompanyId()));
	...
	public List getSerchTicketData(UserConfig userConfig,String sql){
		...
		String hql = "SELECT t.ticketId,t.ticketNumber,t.pawner.title, t.pawner.initials , t.pawner.surName,t.pawner.pawnerCode,t.version,t.ticketStatusId " +
	...
	public Ticket getTicketByTicketNumber(UserConfig userConfig,String number){
		...
		criteria.setFetchMode("reminderMap", FetchMode.JOIN);
		//criteria.add(Restrictions.eq("ticketStatusId", TicketStatusEnum.ACTIVE.getCode()));
...
//		ticketDTO.setInterestSlab(slabList.get(0).getRate()/12);
		...
		ticketDTO.setTicketNo(ticket.getTicketNumber());
		ticketDTO.setTicketStatus(ticket.getTicketStatusId());
...
//				throw new CommonDataAccessException("errors.notauthorize");
	...
	/* (non-Javadoc)
	 * @see com.dass.pawning.dao.TicketDAO#getClientExposure(com.dass.core.util.UserConfig, int)
	 * 
	 */
